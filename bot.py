...
async def start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    txt = "👋 **NDTT STORE - HỆ THỐNG PREMIUM ACCOUNTS**\n\n🛒 Dịch vụ tự động 24/7\n💳 Thanh toán thông minh - Nhận hàng tức thì."
...
async def huong_dan_mua(u: Update, c: ContextTypes.DEFAULT_TYPE):
    txt = (
        "📖 **HƯỚNG DẪN MUA HÀNG**\n━━━━━━━━━━━━━━\n"
        "1️⃣ Sử dụng lệnh /list hoặc nút **Bảng Giá**.\n"
        "2️⃣ Chọn gói dịch vụ và quét mã QR thanh toán.\n"
        "3️⃣ Hệ thống xác nhận và gửi thông tin qua tin nhắn này.\n\n"
        "⚠️ *Lưu ý: Chuyển khoản chính xác nội dung đơn hàng.*"
    )
...
async def nhap_kho(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if items:
            rows = [[p_name, it[0].strip(), it[1].strip() if it[1] else "N/A", "Sẵn sàng", datetime.datetime.now().strftime('%d/%m %H:%M')] for it in items]
...
async def clear_kho(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await u.message.reply_text(f"🗑️ Đã làm trống kho hàng: `{p}`")
    except: pass
...
async def lenh_xoai(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if res: await u.message.reply_text(f"🎁 **QUÀ TẶNG ĐẶC BIỆT**\n🔑 `{res}`", parse_mode='Markdown')
    else: await u.message.reply_text("🔴 Món quà này hiện đã hết hàng.")
...
async def handle_buy(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data.startswith("buy_"):
        p = q.data.replace("buy_", "")
        db = get_db(); data = db.worksheet("DataBot").get_all_records()
        ...
        await q.message.reply_photo(photo=qr, caption=f"💳 **THANH TOÁN ĐƠN HÀNG**\n📦 SP: `{p}`\n💰 Giá: `{pr:,}`đ\n📝 Nội dung: `{oid}`", parse_mode='Markdown')
...
async def p_init(app: Application):
    await app.bot.set_my_commands([
        BotCommand("start", "Khởi động"), BotCommand("list", "Sản phẩm"),
        BotCommand("buy", "Hướng dẫn"), BotCommand("contact", "Hỗ trợ")
    ])
...
def main():
    ...
    bot.add_handler(CommandHandler("contact", lambda u, c: u.message.reply_text("☎️ Admin Support: @NgDanhThanhTrung")))
    bot.add_handler(MessageHandler(filters.Text(["📊 Xem Bảng Giá"]), show_catalog))
    bot.add_handler(MessageHandler(filters.Text(["☎️ Hỗ trợ"]), lambda u, c: u.message.reply_text("☎️ Admin Support: @NgDanhThanhTrung")))
